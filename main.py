...
# 1日分のコロナ感染者情報の送信の処理
def infected_data_reply(event: any, day: int, line_bot_api: any):
    ...
    if type(new_data_array) is str:
        ...
        message.make_quick_message(token=event.reply_token, text=new_data_array, line_bot_api=line_bot_api)
        return
    ...
    message.make_quick_message(token=event.reply_token, text=line_text_new_data, line_bot_api=line_bot_api)
...
def handle_message(event):
    ...
    if text == '最新' or text == '最新情報':
        day = 0
        infected_data_reply(event=event, day=day, line_bot_api=line_bot_api)
    # 一日前の情報を送信する
